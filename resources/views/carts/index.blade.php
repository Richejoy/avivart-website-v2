@extends('layouts.cart', ['title' => 'Accueil'])
					   	...
					   	<tbody>
					   		@foreach($cartContent as $row)
					       		<tr>
					           		...
					           		<td><a href="{{ route('store.show', ['product' => $row->id]) }}">{{ $row->name }}</a></td>
					           		...
					           		<td>
					           			<form method="get" action="{{ route('cart.update', ['row' => $row->rowId]) }}" accept-charset="utf-8" autocomplete="off">
							      			<div class="input-group">
												<input type="number" name="quantity" id="quantity" class="form-control" placeholder="Quantité"  value="{{ $row->qty }}" required>
					           		...
					           		<td>
					           			<a data-toggle="tooltip" title="Retirer du panier" href="{{ route('cart.remove', ['row' => $row->rowId]) }}" class="text-danger"><i class="fa fa-trash"></i></a>
					           		</td>
					       		</tr>
					       	@endforeach
					   	</tbody>
			...
			<aside class="col-lg-4">
				...
				@else
					...
					{!! Form::open(['route' => 'cart.apply_coupon']) !!}
		      			<div class="input-group">
							<input type="text" name="code" id="code" class="form-control" placeholder="Entrez votre code" required>
							<div class="input-group-append">
